...
class ArcAim:
    ...
    def __init__(self, intersection_traj_file, lanes_file):
    	...
    	self.intersection_traj_df = intersection_file
    ...
    def build_intersection(self):
    	...
    	for index, row in self.lanes_df.iterrows():
    		traj = BezierTrajectory.BezierTrajectory(row['TAIL_X'], row['TAIL_Y'], row['MID_X'], row['MID_Y'], row['HEAD_X'], row['HEAD_Y'])
    		if row['IO'] == 'I':
    	...
    	for index, row in self.intersection_traj_df.iterrows():
    		...
    		intersection_lane = IntersectionLane.IntersectionLane(traj)
